...
const OnGoingList = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    ...
    if (tabListStatus.searchValue.onGoing !== "" && tabListStatus.searchValue.onGoing !== null) {
      listRef.current.instance.option('searchValue', tabListStatus.searchValue.onGoing);
    };
    return () => { };

  }, [tabListStatus.searchValue.all, listRef.current]);
  ...
  const storeDataSource = useMemo(() => {
    ...
    return new DataSource({
      ...
      skip: 0,
      take: 30,
      sort: [{ selector: 'DeliveryDate', desc: false },],
      filter,
      searchValue: tabListStatus.searchValue.onGoing || "",
    });
  }, [filter, LoginUser]);
  ...
  // open modal
  const handleOpenAddNewProductivity = () => {
    popupRef.current.present();
  };

  const handleSelectItem = (data) => {
    navigate(PATH_APP.qc.production_activity.detail(data?.Id), { state: data, });
    dispatch(setTabListStatus({
      selectedTabName: '1',
      searchValue: {
        onGoing: listRef.current.instance._searchEditor._changedValue,
        finished: null,
        all: null,
      }
    }))
  ...
  return (
    <div>
      ...
      <FloatButton
        onClick={handleOpenAddNewProductivity}
      />

      <PopupNewProductActivity
        ref={popupRef}
      />
...
function ItemTemplate({ data, navigate, handleSelectItem }) {
  ...
  return (
    <Stack
      //  onClick={() => navigate(PATH_APP.qc.production_activity.detail(data?.Id), { state: data })}
