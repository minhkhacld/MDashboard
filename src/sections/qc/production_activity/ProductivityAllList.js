...
// ----------------------------------------------------------------
const ProductivityAllList = forwardRef((props, ref) => {
    ...
    const defaultFilter = ["ReporterId", "=", LoginUser.EmpId];
    ...
    const storeDataSource = useMemo(() => {
        ...
        return new DataSource({
            ...
            filter,
            searchValue: tabListStatus.searchValue.all || "",
        });
    }, [filter, LoginUser,]);
    ...
    useEffect(() => {
        ...
        if (tabListStatus.searchValue.all !== "" && tabListStatus.searchValue.all !== null) {
            listRef.current.instance.option('searchValue', tabListStatus.searchValue.all);
            filterPanelRef.current.hide();
        };
        // const count = listRef.current?.instance?._dataSource?._totalCount
        // if (count >= 0) {
...
export default ProductivityAllList;
