...
function UploadFileBackDrop({
    ...
    useEffect(() => {
        (async () => {
            ...
            const percentage = Math.round(Number(signalR.message) / 12 * 100);
            ...
            <DialogContent sx={{ minWidth: 280 }}>
                ...
                >
                    <Stack spacing={1} justifyContent="flex-start" alignItems={'center'} width='100%' direction={'row'}>
                        <Typography color={'text.black'} textAlign='center' fontWeight={'bold'}>{currentLang.value === 'en' ? 'Uploading data' : 'Đang tải lên dữ liệu'} :</Typography>
                        <Iconify icon={progress === 100 ? IconName.checked : 'eos-icons:loading'}
                            sx={{
...

                    {
                        ...
                        <>
                            <Stack spacing={1} justifyContent="flex-start" alignItems={'center'} width='100%' direction={'row'}>
                                <Typography color={'text.black'} textAlign='center' fontWeight={'bold'}>{currentLang.value === 'en' ? 'Processing data' : 'Đang xử lý dữ liệu'} :</Typography>
                                <Iconify icon={signalR.message === "4" ? IconName.checked : 'eos-icons:loading'}
                                    sx={{
                    ...
                    <Stack spacing={1} justifyContent="flex-start" alignItems={'center'} width='100%' direction={'row'}>
                        <Typography color={'text.black'} textAlign='left' variant='caption'> {currentLang.value === 'en' ? 'Please keep your internet connection stable...' : 'Vui lòng không chuyển mạng khi đang tải lên dữ liệu'} </Typography>
                    </Stack>
