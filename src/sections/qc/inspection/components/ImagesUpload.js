...
InspectionImagesUpload.propTypes = {
...
function InspectionImagesUpload({ translate, currentInspection, methods, isViewOnly, setLoading, loading, setProgress }) {
  ...
  const handleRemoveAll = () => {
    ...
    const Images = [...values.Images].filter((d) => d.Id > 0 || d.ParentGuid === values?.AfterGuid);

    setValue(
      'Images',
      Images.map((image) => {
        if (image.ParentGuid !== values?.AfterGuid) {
          const newImage = {
            ...image,
            Id: image.Id,
            Action: 'Delete',
  ...
  const handleFilterImages = (file) => file?.ParentGuid === values?.Guid;


  return (
    <Stack>

      <Typography>{`Photos (Before Improvement)`}</Typography>
...
export default InspectionImagesUpload;
